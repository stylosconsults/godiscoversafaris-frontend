...
export const createComment = (data, slug) => async dispatch => {
	const createAcomment = await API.createCommentNews(
		data,
		'/api/comments/create'
	);

	const commentsResponse = await API.getNewsComments(
		`/api/comments/${slug}/news`
	);
...
export const getCommentsNews = slug => async dispatch => {
	const commentsResponse = await API.getNewsComments(
		`/api/comments/${slug}/news`
	);
